...
class SnippetGenerator_Tests(unittest.TestCase):
    ...
    def setUp(self):
        self.df_column_names = ['reel_filename', 'image_filename', 'snip_name', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'x4', 'y4']
    ...
    def test_get_box_coordinates(self):
        ...
        df = pd.DataFrame(data=[row_1, row_2], columns=['x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'x4', 'y4'])
    ...
    def test_yield_snippet_and_name(self):
        ...
        for image_name, image in self.snippet_generator.yield_image_and_name(self.image_tar_path):
            for snip_name, snippet in self.snippet_generator.yield_snippet_and_name(os.path.basename(self.image_tar_path), image_name, image):
                ...
                if test_snippet.size == snippet.size and test_snippet.mode == snippet.mode:
    ...
    def test_save_snippets_as_tar(self):
        ...
        try:
            subprocess.run(['tar', '-xf', os.path.join(out_dir, tarfile_out_filename), '-C', out_dir])
    ...
    def compare_actual_paths_to_expected_paths(self, out_dir: str):
        ...
        for row in df.itertuples():
            ...
            snippet_file = f"{tarfile_name_no_ext}_{image_name_no_ext}_{row.snip_name}.png"
            set_of_snippet_paths.add(os.path.join(out_dir, tarfile_name_no_ext, image_name_no_ext, snippet_file))
